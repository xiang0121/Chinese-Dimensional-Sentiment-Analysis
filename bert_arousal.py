...
def k_fold_cross_validation(model_path,dfs,model_save_path, n_splits=5, batch_size=2, num_epochs=3, learning_rate=2e-5):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(dfs)):
        ...
        train_dataset = RegressionDataset(train_fold['Text'].tolist(), train_fold[['Arousal_Mean', 'Arousal_SD']])
        val_dataset = RegressionDataset(val_fold['Text'].tolist(), val_fold[['Arousal_Mean', 'Arousal_SD']])
        ...
        for epoch in (p:=tqdm(range(num_epochs))):
            ...
            if fold_loss < eval_old_loss:
                torch.save(model.state_dict(), f"{model_save_path}/Arousal_model_state_{fold_loss:.3f}.pth")
...
def get_best_model():
    model_wegiht=glob.glob("./model_weight/bert_regression_model_valence/Arousal*.pth")
    validation_loss=[]
    for i in model_wegiht:
        validation_loss.append(float(re.search(r"Valence_model_state_(\d+.\d+).pth",i).group(1)))

    return f"./model_weight/bert_regression_model_valence/Arousal_model_state_{min(validation_loss)}.pth"
